...
class TrainableFIRReverb(nn.Module):
    ...
    def forward(self, z):
        ...
        INPUT_SIGNAL = torch.view_as_real(torch.fft.rfft(zero_pad_input_signal, dim=-1))
        ...
        FIR = torch.view_as_real(torch.fft.rfft(zero_pad_final_fir, dim=-1))
        ...
        output_signal = torch.fft.irfft(torch.view_as_complex(OUTPUT_SIGNAL.contiguous()), n=zero_pad_input_signal.shape[-1], dim=-1)
