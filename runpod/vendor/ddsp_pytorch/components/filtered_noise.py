...
class FilteredNoise(nn.Module):
    def __init__(self, frame_length = 64, attenuate_gain = 1e-2, device = 'cpu'):
    ...
    def forward(self, z):
        ...
        zero_phase_ir_bank = torch.fft.irfft(torch.view_as_complex(ZERO_PHASE_FR_BANK.contiguous()), n=filter_coeff_length * 2 - 1, dim=-1)
        ...
        ZERO_PADED_WINDOWED_LINEAR_PHASE_FR_BANK = torch.view_as_real(torch.fft.rfft(zero_paded_windowed_linear_phase_ir_bank, dim=-1))
        ...
        ZERO_PADED_NOISE = torch.view_as_real(torch.fft.rfft(zero_paded_noise, dim=-1))
        ...
        filtered_noise = torch.fft.irfft(torch.view_as_complex(FILTERED_NOISE.contiguous()), dim=-1).view(batch_num, frame_num, -1) * self.attenuate_gain         
